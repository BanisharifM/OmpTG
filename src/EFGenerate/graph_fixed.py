                ...
                for ne in nx.all_neighbors(graph, node):
                    if ne.endswith('exit'):
                        ...
                        for nodeName in nx.nodes(graph):
                            if (graph.nodes[nodeName]['label'].split('\n')[-1] == nameToBeFind):
                                ne = nodeName
                                break
                        taiList = []
    ...
    for callBlock in relationDict.keys():
        ...
        if relationDict[callBlock].startswith('ort_'):
            ...
            if relationDict[callBlock] == 'ort_taskwait':
                graph.nodes[callBlock]['style'] = 'filled'
                graph.nodes[callBlock]['color'] = 'green'
            graph.nodes[callBlock]['label'] = callBlock + '\n(' + callBlock.split('__bb')[0] + ')' + relationDict[
                                                                                                        callBlock][4:]
            continue
            ...
            graph.nodes[callBlock]['style'] = 'filled'
            graph.nodes[callBlock]['color'] = 'aquamarine'
...
def connectEdgeForEFG(EFG, relationDict):
    for node in EFG.nodes:
        if ('CREATE' in EFG.nodes[node]['label'] or 'CALL' in EFG.nodes[node]['label']):
